...
public class TenantServiceImpl implements TenantService {
	...
	public void getAccountRoleBind(String username, Long tenantId, String[] roleCodes) {
		//如果已存在账号，判断是否绑定了角色（角色可能是多个），如果没绑定角色，则绑定
		List<AccountRoleDTO> accountRoleDTOS = authorizationService
				.queryAccountBindRole(username, tenantId, roleCodes);
		if (accountRoleDTOS.isEmpty()) {
			...
			List<String> collect = listRoleCodes.stream().filter(item -> !accountRoleDTOS.contains(item))
					.collect(toList());
			if (collect != null) {
				authorizationService
						.bindAccountRole(username, tenantId, collect.toArray(new String[collect.size()]));
			}
		}
	}
